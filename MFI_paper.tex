...
Podejścia hybrydowe integrują wiele modalności naraz (np. tokeny, drzewa AST i grafy CFG) w celu uzyskania kompleksowej reprezentacji kodu źródłowego. Takie rozwiązanie kompensuje wady pojedynczych metod, łącząc wiedzę o sekwencji, gramatyce i logice wykonania.

Doskonałym przykładem tego podejścia jest sieć MMAN (Multi-Modal Attention Network) \cite{b4}. Przetwarza ona każdą reprezentację dedykowanym jej modelem (LSTM dla tokenów, Tree-LSTM dla AST oraz GGNN dla CFG), a następnie wykorzystuje mechanizm uwagi (ang. \textit{attention}). Mechanizm ten przypisuje wagi ($\alpha$) poszczególnym elementom wewnątrz każdej modalności, co pozwala zidentyfikować fragmenty najbardziej istotne dla semantyki programu. Ostateczna, wielomodalna reprezentacja programu $x$ powstaje poprzez konkatenację średnich ważonych z każdego modelu i transformację przez macierz wag $W$ \cite{b4}:
\begin{equation}
